do
local Peyman = 259019897 --put your id here(BOT OWNER ID)
local Peyman = 259019897
--local Peyman2 = 259019897
...
local function setrank(msg, name, value,receiver) -- setrank function
  ...
  if hash then
	...
	return send_msg(receiver, 'کاربر ('..name..') مقام گرفت!\nمقام جدید: '..value, ok_cb,  true)
  end
end
...
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   Username = '----'
    ...
    if not value then
	 if result.peer_id == tonumber(Peyman) then
	   text = text..'👑 مقام: سازنده ربات 👑 \n'
	   elseif result.peer_id == tonumber(Peyman) then
	   text = text..'👑 مقام: سازنده ربات 👑 \n'
 ...
 if result.username then
  ...
  if not value then
	 if result.peer_id == tonumber(Peyman) then
	   ...
	   elseif result.peer_id == tonumber(Peyman) then
	   text = text..'👑 مقام: سازنده ربات 👑 \n'
	   elseif result.peer_id == tonumber(Peyman2) then
	   text = text..'👑 مقام: سازنده ربات 👑 \n'
	  elseif is_admin2(result.peer_id) then
	   text = text..'🔥 مقام: ادمین  \n'
	  elseif is_owner2(result.peer_id, extra.chat2) then
  ...
  send_msg(extra.receiver, 'من نتونستم پیدا کنم :|\nاز دستور\n/info @username\nاستفاده کنید!', ok_cb, false)
  end
end
  ...
  local text = '🔵 نام کامل: '..(result.from.first_name or '')..' '..(result.from.last_name or '')..'\n'
	...
	local hash = 'rank:'..result.to.id..':variables'
		local value = redis:hget(hash, result.from.peer_id)
		 if not value then
		    if result.from.peer_id == tonumber(Peyman) then
		       text = text..'👑 مقام: سازنده ربات 👑 \n'
			   elseif result.peer_id == tonumber(Peyman) then
	           text = text..'👑 مقام: سازنده ربات 👑 \n'
	          --elseif result.peer_id == tonumber(Sosha2) then
  ...
  if msg.from.username then
   ...
   local hash = 'rank:'..msg.to.id..':variables'
	if hash then
	  ...
	  if not value then
		if msg.from.id == tonumber(Peyman) then
		 text = text..'👑 مقام: سازنده ربات 👑 \n'
		 elseif msg.from.id == tonumber(Peyman) then
		 text = text..'👑 مقام: سازنده ربات 👑 \n'
