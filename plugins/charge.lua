...
function run(msg, matches)
	if matches[1]:lower() == 'charge' then
		...
		return "گروه با موفقیت تا #"..matches[2].."روز دیگر شارژ شد."
	end
	if matches[1]:lower() == 'chargetime' then
		local expiretime = redis:hget ('expiretime', get_receiver(msg))
		if not expiretime then return 'تاریخ ست نشده است' else
			local now = tonumber(os.time())
			return (math.floor((tonumber(expiretime) - tonumber(now)) / 86400) + 1) .. " روز دیگر تا پایان شارژ"
		end
	end
